...
bool Session::processRequest(StringPiece request)
{
	...
	else if(command_ == "get" || command_ == "gets")
	{
		...
		{ 
			LOG_DEBUG << "shrink outputbuf from " << conn_->outputBuffer()->internalCapacity();
			conn_->outputBuffer()->shrink(65536 + outputBuf_.readableBytes());
		}
		*/
		conn_->send(&outputBuf_);
		/*
		int remaining = outputBuf_.readableBytes();
		while(remaining > 0)
		{
			if(remaining < Session::kMaxSendLen)
			{
				remaining -= outputBuf_.readableBytes();
				conn_->send(&outputBuf_);
				assert(remaining == 0);
			}
			else
			{
				conn_->send(outputBuf_.peek(),kMaxSendLen);
				outputBuf_.retrieve(kMaxSendLen);
				remaining -= kMaxSendLen;
			}
		}
		*/
	}		
	...
	{
#ifdef HAVE_TCMALLOC
		reply("VERSION 0.01 with tcmalloc\r\n");
#else	
		reply("VERSION 0.01\r\n");
#endif
	}
...
bool Session::doUpdate(Session::Tokenizer::iterator& beg,Session::Tokenizer::iterator end)
{
	...
	//TODO:what if exptime is a super large number?
...
bool Session::doDelete(Session::Tokenizer::iterator& beg,Session::Tokenizer::iterator end)
{
